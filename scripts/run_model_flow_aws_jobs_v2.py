"""Run feature building, segmented training, and batch scoring in SageMaker Processing.

This script writes every run to a versioned S3 prefix, so model registry and
champion/challenger logic can compare model runs before publishing to latest.
"""
...
def make_processor(args: argparse.Namespace) -> FrameworkProcessor:
    ...
    session = sagemaker.Session(boto_session=boto_session, default_bucket=args.bucket)
...
def main() -> None:
    ...
    print("1/3 Build features")
    processor.run(
        code="scripts/sagemaker_entrypoints/build_features.py",
        source_dir=".",
        inputs=[
            ProcessingInput(source=f"s3://{bucket}/{args.sales_key}", destination="/opt/ml/processing/input/sales"),
            ProcessingInput(source=f"s3://{bucket}/{args.test_key}", destination="/opt/ml/processing/input/test"),
            ProcessingInput(source=f"s3://{bucket}/{args.items_key}", destination="/opt/ml/processing/input/items"),
            ProcessingInput(source=f"s3://{bucket}/{args.categories_key}", destination="/opt/ml/processing/input/categories"),
            ProcessingInput(source=f"s3://{bucket}/{args.shops_key}", destination="/opt/ml/processing/input/shops"),
        ],
        outputs=[ProcessingOutput(source="/opt/ml/processing/output", destination=features_s3)],
        arguments=[
            ...
            "--validation-block", str(args.validation_block),
            "--inference-block", str(args.inference_block),
        ],
        wait=True,
        logs=True,
    )

    print("2/3 Train segmented models")
    processor.run(
        code="scripts/sagemaker_entrypoints/train_segmented.py",
        source_dir=".",
        inputs=[ProcessingInput(source=features_s3, destination="/opt/ml/processing/features")],
        outputs=[
    ...
    processor.run(
        ...
        outputs=[ProcessingOutput(source="/opt/ml/processing/predictions", destination=predictions_s3)],
        arguments=[
