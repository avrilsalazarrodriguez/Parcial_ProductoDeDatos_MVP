...
def predict_with_model(model_payload: dict, features: pd.DataFrame) -> np.ndarray:
    ...
    El modelo ya viene de las tareas previas:
...
def build_evaluation_sample(valid_df: pd.DataFrame, model_payload: dict, n: int = 3000) -> pd.DataFrame:
    """
    Aqui generamos una muestra de evaluación para comparar predicción vs ground truth.

    Usamos una muestra para que la app responda rápido y no cargue millones de filas.
    """
...
def rmse(y_true: pd.Series, y_pred: pd.Series) -> float:
    """Aqui calculamos RMSE para reportar el error del modelo."""
    return float(np.sqrt(np.mean((y_true - y_pred) ** 2)))


def naive_prediction(valid_df: pd.DataFrame) -> pd.Series:
    """
    Aqui construimos un baseline naive simple.
