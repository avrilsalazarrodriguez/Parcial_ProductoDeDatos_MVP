...
from backend.db import (
    ...
    read_usage_events as _read_usage_events,
)
...
def _log_nonfatal_rds_error(action: str, exc: Exception) -> None:
    """Registra errores de RDS sin romper la UI local de Streamlit."""
    LOGGER.warning(
...
def insert_usage_event(
    ...
    """Registra eventos de uso si RDS está disponible.

    En local, si `DISABLE_RDS_WRITES=true`, no intenta conectarse a RDS.
    Esto evita que la app se caiga por no encontrar el stack `pfs-mvp-rds`.
    """
    ...
    try:
        ...
        _log_nonfatal_rds_error("insert_usage_event", exc)
        return False
...
def insert_batch_export(
    ...
    """Registra el historial del archivo CFO si RDS está disponible."""
    if DISABLE_RDS_WRITES:
        return False

    try:
        _insert_batch_export(
            scope=scope,
            shop_id=shop_id,
            records_count=records_count,
        ...
        _log_nonfatal_rds_error("insert_batch_export", exc)
        return False


def read_batch_exports(limit: int = 20) -> pd.DataFrame:
    """Lee historial de exports; regresa vacío si RDS no está disponible."""
    if DISABLE_RDS_WRITES:
        return pd.DataFrame()
    ...
    try:
        ...
        _log_nonfatal_rds_error("read_batch_exports", exc)
        return pd.DataFrame()
...
def insert_business_feedback(
    ...
    """Guarda feedback de negocio si RDS está disponible."""
    if DISABLE_RDS_WRITES:
        raise RuntimeError(
            "RDS está desactivado para pruebas locales "
            "(DISABLE_RDS_WRITES=true). El feedback no se guardó."
        )

    _insert_business_feedback(
        shop_id=shop_id,
        item_id=item_id,
...
def read_business_feedback(limit: int = 100) -> pd.DataFrame:
    """Lee feedback de negocio; regresa vacío si RDS no está disponible."""
    if DISABLE_RDS_WRITES:
        return pd.DataFrame()
    ...
    try:
        ...
        _log_nonfatal_rds_error("read_business_feedback", exc)
        return pd.DataFrame()


def read_problem_products(limit: int = 100) -> pd.DataFrame:
    """Lee productos problemáticos; regresa vacío si RDS no está disponible."""
    if DISABLE_RDS_WRITES:
        return pd.DataFrame()
    ...
    try:
        ...
        _log_nonfatal_rds_error("read_problem_products", exc)
        return pd.DataFrame()


def read_usage_events(limit: int = 100) -> pd.DataFrame:
    """Lee eventos de uso; regresa vacío si RDS no está disponible."""
    if DISABLE_RDS_WRITES:
        return pd.DataFrame()

    try:
        return _read_usage_events(limit=limit)
    except Exception as exc:  # noqa: BLE001
        _log_nonfatal_rds_error("read_usage_events", exc)
        return pd.DataFrame()
...
with st.sidebar:
    ...
    st.write(
        "RDS:",
        "desactivado en local" if DISABLE_RDS_WRITES else "activo",
    )
    if DISABLE_RDS_WRITES:
        st.caption(
            "Prueba local: la app no intentará escribir o leer RDS. "
            "En ECS/Fargate debe usarse DISABLE_RDS_WRITES=false."
        )
...
with tab3:
    ...
    st.dataframe(filtered_batch.head(1000), width="stretch")
    ...
    if st.button("Generar archivo CFO y guardar en S3"):
        ...
        rds_registered = insert_batch_export(
            scope=scope,
            shop_id=shop_for_s3,
        ...
        if rds_registered:
            ...
            st.warning(
                "No se registró historial en RDS. "
                "Esto es esperado si estás probando localmente con "
...
with tab5:
    ...
    col1.plotly_chart(fig_shop_error, width="stretch")
    ...
    col2.plotly_chart(fig_item_error, width="stretch")

    st.subheader("Error por tienda")
    st.dataframe(by_shop, width="stretch")

    st.subheader("Productos con mayor error")
    st.dataframe(by_item.head(50), width="stretch")
    ...
    st.dataframe(low_activity_products, width="stretch")
    if USE_MODELOPS_S3:
        st.divider()
        render_modelops_registry_block()
